...
public:
  ...
  LogicalResult matchAndRewrite(AtenRepeatOp op,
    ...
    Value one = rewriter.create<Torch::ConstantIntOp>(
        loc, rewriter.getI64IntegerAttr(1));
    int64_t leadingRank = repeats.size() - rank;
    for (int i = 0; i < leadingRank; ++i) {
    ...
    for (int i = 0; i < rank; i++) {
      ...
      newSizes0.push_back(one);
      newSizes0.push_back(dimSize);
      newSizes1.push_back(scale);
      newSizes1.push_back(dimSize);
    ...
    Type reshapeType2 =
        ValueTensorType::get(context, llvm::makeArrayRef(newShape2), dtype);
